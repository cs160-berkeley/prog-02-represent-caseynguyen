...
public abstract class JSONEntity {
	 ...
	 * @return
	 */
	public static JSONObject[] getJSONObjects(ApiCall call, Map<String, String> params, String apiCall, String arrayName) {
		try {
			JSONObject obj = call.getJSONResponse(apiCall, params);
			// JSONArray items = obj.getJSONArray(arrayName);
	 ...
	 * @return
	 */
	public static JSONObject[] getJSONObjects2(ApiCall call, 
			Map<String, String> params, String apiCall, String arrayName) {
		try {
			JSONObject obj = call.getJSONResponse2(apiCall, params);
			JSONArray items = obj.getJSONArray("results");
			List<JSONObject> holder = new ArrayList<JSONObject>();
			for (int i = 0; i < items.length(); i++) {
				JSONObject js = (JSONObject) items.get(i);
	 ...
	 * @return
	 */
	public static JSONObject getJSONObject(ApiCall call, Map<String, String> params, String apiCall, String arrayName) {
		try {
			JSONObject obj = call.getJSONResponse(apiCall, params);
...
//			}
	 ...
	 * @param out non-null appender
	 */
	public void show(Appendable out, int index) {
		try {
			out.append(String.format("#%d - %s", index, getClass().getSimpleName()));
			out.append("\n");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
